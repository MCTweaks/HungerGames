...
public class GameListener implements Listener {
    ...
    private void onChestUse(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && playerManager.hasPlayerData(player)) {
            ...
            if (gameManager.getGame(player.getLocation()).getGameArenaData().getStatus() == Status.COUNTDOWN || gameManager.getGame(player.getLocation()).getGameArenaData().getStatus() == Status.WAITING)
                event.setCancelled(true);
